...
node_1(){
	...
	[[ "${node}" == "1" ]] && ISP_name="上海电信" && ip=101.227.255.45
	[[ "${node}" == "2" ]] && ISP_name="厦门电信" && ip=117.28.254.129
	[[ "${node}" == "3" ]] && ISP_name="襄阳电信" && ip=58.51.94.106
	[[ "${node}" == "4" ]] && ISP_name="南昌电信" && ip=182.98.238.226
	[[ "${node}" == "5" ]] && ISP_name="成都电信" && ip=61.188.6.210
	[[ "${node}" == "6" ]] && ISP_name="广州电信" && ip=14.215.116.1
	[[ "${node}" == "7" ]] && ISP_name="昆明电信" && ip=116.55.247.129
}
node_2(){
	echo -e "\n1.拉萨联通\n2.重庆联通\n3.郑州联通\n4.合肥联通\n5.南京联通\n6.杭州联通\n7.昆明联通" && read -p "输入数字以选择:" node
	...
	[[ "${node}" == "1" ]] && ISP_name="拉萨联通" && ip=221.13.70.244
	[[ "${node}" == "2" ]] && ISP_name="重庆联通" && ip=113.207.32.65
	[[ "${node}" == "3" ]] && ISP_name="郑州联通" && ip=61.168.23.74
	[[ "${node}" == "4" ]] && ISP_name="合肥联通" && ip=112.122.10.26
	[[ "${node}" == "5" ]] && ISP_name="南京联通" && ip=58.240.53.78
	[[ "${node}" == "6" ]] && ISP_name="杭州联通" && ip=101.71.241.238
	[[ "${node}" == "7" ]] && ISP_name="昆明联通" && ip=14.204.149.193
}
node_3(){
	echo -e "\n1.上海移动\n2.成都移动\n3.合肥移动\n4.杭州移动\n5.昆明移动" && read -p "输入数字以选择:" node
	...
	[[ "${node}" == "2" ]] && ISP_name="成都移动" && ip=183.221.253.66
	[[ "${node}" == "3" ]] && ISP_name="合肥移动" && ip=120.209.140.60
	[[ "${node}" == "4" ]] && ISP_name="杭州移动" && ip=112.17.0.106
	[[ "${node}" == "5" ]] && ISP_name="昆明移动" && ip=39.130.140.2
}
node_4(){
...
test_all(){
	result_all	'101.227.255.45'	'上海电信'
	
        result_all	'117.28.254.129'	'厦门电信'

	result_all	'101.71.241.238'	'杭州联通'

	result_all	'112.17.0.106'		'杭州移动'
